...
export const incidentLogSchema = z
  .object({
    ...
    incidentTypeDetail: z.string().trim().optional(),
    locationZone: z.string().trim().min(1, "Indique el local o zona"),
    description: z.string().trim().min(1, "Describa la incidencia"),
  })
  .refine((data) => data.incidentType !== "Otro" || (data.incidentTypeDetail?.length ?? 0) > 0, {
    message: "Especifique el tipo de incidencia",
    path: ["incidentTypeDetail"],
  });
