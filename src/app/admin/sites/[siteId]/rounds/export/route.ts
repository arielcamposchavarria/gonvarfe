...
import { buildSiteRoundsSheet } from "@/lib/export/site-report-sheets";
import { parseDateRangeParams } from "@/lib/date-range";

export const dynamic = "force-dynamic";

export async function GET(request: Request, { params }: RouteContext<"/admin/sites/[siteId]/rounds/export">) {
  try {
    await requireAdmin();
  } catch {
  ...
  const rounds = await container.listRoundsBySite(siteId, range);

  return buildReportResponse(
    parseReportFormat(request),
    [buildSiteRoundsSheet(rounds)],
    `${slugifyFilename(sitio.nombre)}-recorridos`,
  );
}
